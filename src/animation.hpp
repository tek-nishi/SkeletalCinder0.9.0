...
QuatKey fromAssimp(const aiQuatKey& key) {
  ...
  v.time  = key.mTime;
  v.value = ci::quat{ key.mValue.w, key.mValue.x, key.mValue.y, key.mValue.z };
...
ci::vec3 getLerpValue(const double time, const std::vector<VectorKey>& values) {
  // 適用キー位置を探す
  ...
  if (result == values.begin()) {
    ...
    value = glm::mix((result - 1)->value, result->value, float(t / dt));
  }

  return value;
}

ci::quat getLerpValue(const double time, const std::vector<QuatKey>& values) {
  auto result = std::upper_bound(values.begin(), values.end(),
                                 time, Comp<QuatKey>());
  ...
  if (result == values.begin()) {
    ...
    double dt = result->time - (result - 1)->time;
    double t = time - (result - 1)->time;
    value = glm::slerp((result - 1)->value, result->value, float(t / dt));
  }
